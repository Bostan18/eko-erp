  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm font-body text-gray-500">
        ...
        <span>/</span>
        <span className="text-gray-800">{employe.nom_complet}</span>
      </div>
      ...
      <div className="card p-6 flex items-start justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="font-display font-bold text-gray-900 text-xl">{employe.nom_complet}</h1>
            <p className="font-body text-gray-500 text-sm">{employe.poste || 'Poste non défini'}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="card p-5">
          <p className="font-display text-xs font-medium text-gray-500 uppercase tracking-wide mb-3">Informations</p>
          <dl className="space-y-2 text-sm font-body">
            ...
            <div className="flex justify-between">
              ...
              <dd className="text-gray-700">{employe.telephone || '—'}</dd>
            </div>
            <div className="flex justify-between">
              <dt className="text-gray-500">Date d'entrée</dt>
              <dd className="text-gray-700">{employe.date_entree || '—'}</dd>
            </div>
            {employe.taux_journalier && (
              <div className="flex justify-between">
                <dt className="text-gray-500">Taux journalier</dt>
                <dd className="font-semibold text-gray-800">{fmt(employe.taux_journalier)} F</dd>
              </div>
            )}
            {employe.salaire_mensuel && (
              <div className="flex justify-between">
                <dt className="text-gray-500">Salaire mensuel</dt>
                <dd className="font-semibold text-gray-800">{fmt(employe.salaire_mensuel)} F</dd>
              </div>
            )}
          </dl>
        </div>
      ...
      <div className="card overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          <p className="font-display font-semibold text-gray-800 text-sm">Historique présences</p>
          <input
            type="month"
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-50">
            {presences.length === 0 ? (
              ...
              presences.map((p) => (
                <tr key={p.id} className={p.present ? 'hover:bg-gray-50' : 'bg-gray-50 opacity-60'}>
                  <td className="px-4 py-3 font-display font-medium text-gray-700">{p.date}</td>
                  <td className="px-4 py-3">
                    <span className={p.present ? 'badge-green' : 'badge-red'}>
                      {p.present ? 'Présent' : 'Absent'}
                    </span>
                  </td>
                  <td className="px-4 py-3 font-body text-gray-600">{p.heures_travaillees}h</td>
                  <td className="px-4 py-3 font-display font-semibold text-gray-700">
                    {p.present ? `${fmt(p.montant_du)} F` : '—'}
                  </td>
                  <td className="px-4 py-3 font-body text-gray-500">{p.projet_ref || '—'}</td>
                  <td className="px-4 py-3 font-body text-gray-400 text-xs">{p.notes || '—'}</td>
                </tr>
              ))
            )}
          </tbody>
